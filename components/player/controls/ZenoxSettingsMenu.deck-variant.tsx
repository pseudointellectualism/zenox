...
function renderSubtitleSourceBadge(
  ...
  if (srcLower.includes("wyzie")) {
    ...
    badgeLabel = "MEOWTV";
  } else if (srcLower.includes("source")) {
    badgeClasses = active
      ? "bg-black/20 text-black font-bold"
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait" custom={direction} initial={false}>
        {/* ============================================================ */}
        {/* MAIN VIEW: BESPOKE ZENOX STREAM DECK & EXPERIENCE CONTROLS   */}
        {/* ============================================================ */}
        {view === "main" && (
          <motion.div
            key="main"
          ...
          >
            {/* Zenox Stream Deck (3-Item Quick Bar) */}
            <div className="grid grid-cols-3 gap-1 rounded-xl border border-white/10 bg-white/[0.04] p-1.5 backdrop-blur-md">
              {/* Quality Pill */}
              <button
                type="button"
                onClick={() => navigateTo("quality")}
                className="group flex flex-col items-center justify-center rounded-lg py-2 px-1 text-center transition-all hover:bg-white/10 active:scale-95"
              >
                <div className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-white/50 group-hover:text-white/80">
                  <span className="rounded bg-primary/25 px-1 py-0.2 text-[8px] font-black text-primary">
                    HQ
                  </span>
                  <span>Quality</span>
                </div>
                <div className="mt-1 max-w-full truncate text-[13px] font-bold tracking-tight text-white group-hover:text-primary transition-colors">
                  {currentQualityLabel}
                </div>
              </button>
              ...
              <button
                ...
                className="group flex flex-col items-center justify-center rounded-lg border-x border-white/10 py-2 px-1 text-center transition-all hover:bg-white/10 active:scale-95"
              >
                <div className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-white/50 group-hover:text-white/80">
                  <Server className="size-3 text-white/70" />
                  <span>Server</span>
                </div>
                <div className="mt-1 max-w-full truncate text-[13px] font-bold tracking-tight text-white group-hover:text-primary transition-colors">
              ...
              <button
                ...
                className={`group flex flex-col items-center justify-center rounded-lg py-2 px-1 text-center transition-all ${hasMultipleAudio
            ...
            {/* Subtitles Interactive Hub Card */}
            <div className="rounded-xl border border-white/10 bg-white/[0.04] p-2.5 transition-all hover:border-white/20">
              <div className="flex items-center justify-between gap-2">
                ...
                >
                  <div className="grid size-8 shrink-0 place-items-center rounded-lg bg-white/10 text-white transition-colors group-hover:bg-primary group-hover:text-on-primary">
                    <Subtitles className="size-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <span className="text-label-sm font-bold text-white">Subtitles</span>
                      {isSubtitlesEnabled && (
                    ...
                    <p className="truncate text-[11px] text-white/50">
                      {isSubtitlesEnabled ? currentSubtitleLabel : "Turned off"}
                    </p>
                  </div>
                  <ChevronRight className="size-4 text-white/30 transition-all group-hover:translate-x-0.5 group-hover:text-white" />
                </button>

                <div className="flex shrink-0 items-center gap-1.5 border-l border-white/10 pl-2">
                  <button
                    type="button"
                    onClick={() => navigateTo("subtitles-customize")}
            ...
            <div className="flex flex-col gap-0.5 pt-0.5">
              ...
              <button
                ...
                className="group flex w-full items-center justify-between rounded-xl px-3 py-2 text-label-md font-semibold text-white/90 transition-all hover:bg-white/10 hover:text-white active:scale-[0.99]"
              >
                <div className="flex items-center gap-3">
                  <div className="grid size-7 place-items-center rounded-lg bg-white/[0.06] text-white/70 group-hover:text-white transition-colors">
                    <Gauge className="size-4" />
                  </div>
                  <span>Playback & Speed</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="rounded-md bg-white/10 px-2 py-0.5 text-[11px] font-bold text-white/70">
                    {playbackRate}x
                  </span>
                  <ChevronRight className="size-4 text-white/40 transition-transform group-hover:translate-x-0.5 group-hover:text-white" />
                </div>
              </button>
        ...
        {view === "subtitles" && (
          ...
          >
            ...
            <div className="max-h-[44vh] overflow-y-auto pr-0.5 flex flex-col gap-1">
        ...
        {view === "subtitles-group" && (
          ...
          >
            ...
            <div className="max-h-[46vh] overflow-y-auto pr-0.5 flex flex-col gap-1">
              {groupedSubtitles
                .find(
                  (g) =>
        ...
        {view === "subtitles-customize" && (
          <motion.div
            ...
            className="flex flex-col gap-1"
          >
            <ZenoxSubtitleCustomizer onBack={navigateBackToSubtitles} />
          </motion.div>
        )}
